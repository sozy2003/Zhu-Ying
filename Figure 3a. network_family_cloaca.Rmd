---
title: "co-occurrence network in cloaca samples"
author: "Zhu Ying"
date: "2023-06-14"
...
output_prefix = "net_family_cloaca_"
...
table =read.table(otu_abundance_file,header = T, row.names = 1)
...
group_name <- "cloaca"
design_sub <- design[design$group==group_name,]
...
p <- corr.test(t(myData), adjust="BH")  #计算p值
pv <- p$p   #提取p值
...
pear.trim[which(pear.trim[,3] > 0),"class"] <- 1     #1 -> positve
...
node_cloaca <- node
table_family <- table
table_family_shortname <- temp
p_cloaca <- plot(g)

#variable for cloaca samples only
save( arc_cloaca, node_cloaca,g_cloaca,p_cloaca,file=paste0(output_prefix, ".Rdata"))


#variable for 4 groups 
save(table_family_shortname,table_family,
     design,unassigned,vip_species_list, file="common_network.Rdata")
